...
- 🔥 **토큰 타는 게 보여요**: 위젯을 열면 지금 작업이 태우는 토큰이 실시간 그래프로 올라가요. 불꽃은 최근 1분 동안 많이 탈수록 빨리 흔들려요. 카테고리를 누르면 할 거리 목록이 나와요.
...
- 🐣 **마스코트 키우기**: 위젯에서 논 시간과 고른 퀘스트만큼 경험치가 쌓여요. 레벨이 오르면 리본·안경·모자·왕관 같은 꾸미기가 열려요.
...
- 🤝 **Claude·Codex 같이 써도 창은 하나**: 누가 몇 개 작업 중인지 표시해요. Claude는 끝나면 소리로 알려주고, Codex는 조용해지면 알림 없이 창만 접어요 — Codex는 긴 명령을 실행하는 동안과 끝났을 때가 로컬에서 구별되지 않아서, 틀린 완료 알림을 띄우지 않기로 했어요.
...
훅, MCP 서버 등록, 로그인 항목, 마스코트 앱을 지워요. 바꾸는 파일은 백업하고, 저장소 폴더와 `data/` 기록은 남겨둬요.
