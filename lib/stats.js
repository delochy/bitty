...
function codexQuery(threadId, columns) {
  const db = codexDb();
  if (!db || !/^[0-9a-f-]+$/i.test(threadId)) return null;
  try {
    const out = require('child_process').execFileSync('sqlite3', ['-readonly', db,
      `select ${columns} from threads where id = '${threadId}'`], { encoding: 'utf8', timeout: 1500 });
...
/** 그 Codex 세션이 지금까지 태운 토큰 누적 (못 읽으면 null). */
function codexTokensUsed(threadId) {
  const out = codexQuery(threadId, 'tokens_used');
  const used = Number(out);
  return out != null && Number.isFinite(used) ? used : null;
}

/**
 * 24차: 그 Codex 세션이 아직 움직이고 있는지 보는 표식 (달라지면 활동한 것).
 * 값이 무슨 뜻인지는 중요하지 않고 "바뀌었나"만 본다 — lib/state.js settleCodexPending.
 * 토큰만 보면 부족하다: 긴 명령을 실행하는 동안엔 토큰이 안 오르는데, updated_at_ms는
 * 그때도 5~15초마다 갱신된다(실측). 그래서 둘을 합쳐서 본다.
 */
function codexActivityMark(threadId) {
  return codexQuery(threadId, "coalesce(updated_at_ms,0) || '/' || coalesce(tokens_used,0)");
}

function codexPoints(threadId) {
  const used = codexTokensUsed(threadId);
  if (used == null) return [];
...
function liveTokens(working) {
  ...
  for (const w of working) {
    ...
    for (const [at, n] of pts) {
      ...
      events.push([at, n, w.tool]);
      byTool[w.tool] += n;
    }
  }
  if (since === Infinity) {
    return { since: null, total: 0, byTool, recent: 0, series: [], byToolSeries: { claude: [], codex: [] } };
  }
  events.sort((a, b) => a[0] - b[0]);
  let cum = 0, recent = 0;
  const series = [[since, 0]];
  const byToolSeries = { claude: [[since, 0]], codex: [[since, 0]] };
  const toolTotals = { claude: 0, codex: 0 };
  for (const [at, n, tool] of events) {
    cum += n;
    series.push([at, cum]);
    toolTotals[tool] += n;
    byToolSeries[tool].push([at, toolTotals[tool]]);
    if (at >= now - 60 * 1000) recent += n;
  }
  series.push([now, cum]);
  for (const tool of ['claude', 'codex']) byToolSeries[tool].push([now, toolTotals[tool]]);
  // 점이 많으면 고르게 솎는다 (처음과 끝은 남긴다)
  const MAX = 60;
  const thin = (points) => points.length <= MAX ? points
    : points.filter((_, i) => i === 0 || i === points.length - 1 || i % Math.ceil(points.length / MAX) === 0);
  return {
    since,
    total: cum,
    byTool,
    recent,
    series: thin(series),
    byToolSeries: { claude: thin(byToolSeries.claude), codex: thin(byToolSeries.codex) },
  };
}
...
module.exports = { appendActivity, dayStats, mascot, todayKey, gameTotals, todayTokens, liveTokens, codexTokensUsed, codexActivityMark, ITEMS, QUESTS };
