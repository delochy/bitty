...
function saveState(state) {
  try {
    // 24차: 임시 파일에 쓰고 바꿔치기한다 — 읽는 쪽(위젯은 2초마다 읽는다)이 반쯤 쓰인
    // 파일을 보지 않도록.
...
// 24차: 이 파일은 여러 프로세스가 같이 쓴다 — Claude/Codex가 띄우는 MCP 서버가 세션마다
// 하나씩(실제로 8개까지 봤다) 있고, 위젯 데몬도 있다. 예전엔 각자 "읽고 → 고치고 → 통째로
// 저장"해서, 읽은 뒤 저장하기 전에 다른 쪽이 새 세션을 추가하면 그 세션이 통째로 사라졌다
// (실제로 Codex 작업 3개 중 1개가 목록에서 없어졌다). 이제 폴더 만들기를 잠금으로 써서
// (같은 이름 폴더는 하나만 만들어지는 걸 이용) 한 번에 하나씩만 고치게 한다.
...
/** 잠금을 잡고 "최신을 다시 읽어서" 고친 뒤 저장한다. fn이 false를 돌려주면 저장하지 않는다. */
function mutateState(fn) {
  for (let i = 0; i < 60; i++) {
    ...
    try {
      ...
      const out = fn(state);
      if (out !== false) saveState(state);
      return out;
    } finally {
      try { fs.rmdirSync(LOCK_DIR); } catch (e) {}
    }
  }
  // 600ms를 기다려도 못 잡았으면 그냥 진행한다 — 상태 파일 하나 때문에 훅이 막히면 안 된다
  const state = loadState();
  const out = fn(state);
  if (out !== false) saveState(state);
  return out;
...
function reportEvent({ source, event, sessionId, raw, context }) {
  ...
  const now = Date.now();
  // 24차: 아래 계산은 전부 잠금 안에서 — 최신 파일을 다시 읽어서 고치고 저장한다
  ...
  const codexResume =
    nextState === 'WORKING' && prev && prev.pendingDoneAt && String(source).startsWith('codex');
  const continuing =
    codexResume || (nextState === 'WORKING' && transitioned && prev && prev.state === 'DONE' &&
  ...
  if (nextState === 'WORKING' && (transitioned || codexResume)) {
    if (!continuing) {
...
function settleCodexPending(sessionId, mark) {
  const now = Date.now();
  let finished = null; // 잠금을 푼 뒤에 기록한다
  const result = mutateState((state) => {
  const rec = state.sessions[sessionId];
  if (!rec || !rec.pendingDoneAt) return false;
  ...
  if (now - rec.pendingDoneAt < CODEX_IDLE_MS) return false;
  ...
  rec.lastEvent = 'Stop';
  rec.updatedAt = now;
  finished = { source: rec.source, cwd: rec.cwd, startedAt: rec.startedAt, timeBudget: rec.timeBudget, estimate: rec.estimate, workedMs };
  return 'done';
  });
  if (finished) {
    appendJsonl(WORKLOG, {
      ...
      state: 'DONE',
      cwd: finished.cwd,
      startedAt: finished.startedAt ? new Date(finished.startedAt).toISOString() : null,
      workedMs: finished.workedMs,
      promptPreview: null,
      timeBudget: finished.timeBudget,
      estimate: finished.estimate ? finished.estimate.size : null,
    });
  }
  return result === false ? 'waiting' : result;
}
...
function endSessionsWhere(matchSource, reason) {
  ...
  return ended.length > 0; // 바뀐 게 없으면 저장도 하지 않는다
  });
  for (const line of logs) appendJsonl(WORKLOG, line);
  return ended;
}
...
function recordTimeBudget({ sessionId, key, quests }) {
  ...
  mutateState((state) => {
    if (!state.sessions[sid]) return false;
...
function recordEstimate({ sessionId, size, reason }) {
  ...
  mutateState((state) => {
    ...
    if (!rec || rec.state !== 'WORKING') return false;
    rec.estimate = { size, reason: reason ? String(reason).slice(0, 200) : null, at: Date.now() };
    saved = { reason: rec.estimate.reason, turnSeq: rec.turnSeq };
  });
