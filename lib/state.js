...
function reportEvent({ source, event, sessionId, raw, context }) {
  ...
  if (codexPending) {
    ...
    sessionRecord.pendingTokens = prev.pendingTokens != null ? prev.pendingTokens : null;
  } else if (nextState === 'WORKING') {
    delete sessionRecord.pendingDoneAt; // 다시 일을 시작했으니 "끝난 것 같음"은 취소
    delete sessionRecord.pendingTokens;
  }
...
// tokens(그 세션이 지금까지 태운 토큰 누적, 데몬이 Codex 기록에서 읽어 넣어준다)가
// 더 올랐으면 아직 일하는 중이니 시계를 다시 맞추고, 그대로면 CODEX_IDLE_MS 뒤에 끝으로 본다.
// 토큰을 못 읽으면(옛 Codex, 다른 저장 방식) 시간만 보고 판정한다.
const CODEX_IDLE_MS = 45 * 1000;
...
function pendingCodexSessions() {
  ...
  return Object.entries(state.sessions)
    ...
    .map(([sessionId, r]) => ({ sessionId, pendingDoneAt: r.pendingDoneAt, pendingTokens: r.pendingTokens }));
}

/** @returns 'working' | 'done' | 'waiting' — done이면 이때 DONE으로 굳혔다는 뜻. */
function settleCodexPending(sessionId, tokens) {
  const state = loadState();
  const rec = state.sessions[sessionId];
  if (!rec || !rec.pendingDoneAt) return 'waiting';
  const now = Date.now();

  if (tokens != null && rec.pendingTokens != null && tokens > rec.pendingTokens) {
    // 아직 일하는 중 — 끝난 것 같다는 판단을 취소하고 다시 센다
    rec.pendingTokens = tokens;
    rec.pendingDoneAt = now;
    rec.updatedAt = now;
    saveState(state);
    return 'working';
  }
  if (tokens != null && rec.pendingTokens == null) {
    rec.pendingTokens = tokens; // 첫 확인 — 기준값만 적어 둔다
    saveState(state);
    return 'waiting';
  }
...
function endSessionsWhere(matchSource, reason) {
  ...
  for (const [sid, rec] of Object.entries(state.sessions)) {
    ...
    const { pendingDoneAt, pendingTokens, ...clean } = rec;
