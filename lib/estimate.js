...
// 19차: 한국어 원문만 돌려준다 — 번역은 화면 쪽 i18n.js가 한다.
function headlineFor(info) {
  if (!info) return '지금은 쉬는 중이에요';
  if (info.escalated) return '생각보다 길어지고 있어요 ⏳';
  if (!info.estimated) return '작업이 길어지고 있어요 ⏳';
  return HEADLINES[info.level];
}
